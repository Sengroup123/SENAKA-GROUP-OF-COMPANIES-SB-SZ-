'use strict';
/* ═══════════════════════════════════════════════
   SENAKA GROUP DASHBOARD — app.js v6.0
...
const PAGE_TITLES = {
  ...
  finance:'🏛️ Finance Liability', tender:'🏗️ Tender / Production',
...
/* ─── THEME ─── */
const C = {
  bg:'#100c18', card:'#1a0e28', primary:'#7c3a7e', sec:'#5e2d6e',
  accent:'#c47a8a', text:'#f0e8ee', muted:'#b8a8b8',
  green:'#34d399', amber:'#fbbf24', red:'#f87171', gray:'#64748b', blue:'#3b82f6',
  grid:'rgba(94,45,110,0.10)', tick:'#8a7a8a',
  PALETTE:['#7c3a7e','#5e2d6e','#c47a8a','#3d1f52','#e89870',
           '#a0507a','#f5e87a','#d46882','#8b3a88','#b85d7a',
           '#e8a07a','#9b3a90','#dba8b0','#f0d870']
};
const KPI_RGB = { green:'52,211,153', amber:'251,191,36', red:'248,113,113',
                  blue:'59,130,246', purple:'124,58,126', primary:'124,58,126' };
...
function initBgCanvas() {
  ...
  function draw() {
    ...
    for (let i=0; i<dots.length; i++) {
      ...
      ctx.fillStyle='rgba(94,45,110,0.30)'; ctx.fill();
    }
    ctx.lineWidth=.5;
    for (let i=0; i<dots.length; i++) {
      for (let j=i+1; j<dots.length; j++) {
        ...
        if (dist<110) {
          ...
          ctx.strokeStyle='rgba(124,58,126,0.70)';
...
function renderKPIs(containerId, cards) {
  ...
  el.innerHTML=cards.map((c,i)=>{
    ...
    const rgb=KPI_RGB[col]||'37,99,235';
    return `<div class="kpi-card" style="--kpi-color:var(--${col});--kpi-rgb:${rgb};--stagger:${i}">
...
const Charts = {
  _base() {
    return {
      ...
      plugins:{
        legend:{ labels:{ color:C.text, font:{family:"Georgia,'Times New Roman',serif",size:10}, padding:14, usePointStyle:true, pointStyleWidth:8 }},
        tooltip:{
          backgroundColor:'rgba(13,21,38,0.97)', borderColor:'rgba(124,58,237,0.45)', borderWidth:1,
          titleColor:C.accent, bodyColor:C.text,
          titleFont:{family:"Georgia,'Times New Roman',serif",size:11},
          bodyFont:{family:"Georgia,'Times New Roman',serif",size:11},
  ...
  bar(id, labels, datasets, opts={}) {
    ...
    const axisStyle={ grid:{color:C.grid}, ticks:{color:C.tick,font:{family:"Georgia,'Times New Roman',serif",size:9}}, border:{display:false}};
  ...
  polar(id, labels, data, opts={}) {
    ...
    cfg.scales={ r:{ grid:{color:C.grid}, ticks:{color:C.tick,font:{family:"Georgia,'Times New Roman',serif",size:9},backdropColor:'transparent'}, pointLabels:{color:C.muted,font:{family:"Georgia,'Times New Roman',serif",size:10}}}};
  ...
  scatter(id, datasets, opts={}) {
    ...
    const axisStyle={ grid:{color:C.grid}, ticks:{color:C.tick,font:{family:"Georgia,'Times New Roman',serif",size:9}}, border:{display:false}};
    cfg.scales={
      x:{...axisStyle, title:{display:true,text:opts.xLabel||'X',color:C.muted,font:{family:"Georgia,'Times New Roman',serif",size:9}}},
      y:{...axisStyle, title:{display:true,text:opts.yLabel||'Y',color:C.muted,font:{family:"Georgia,'Times New Roman',serif",size:9}}}
    };
...
async function initBankPage() {
  ...
  // Company slicer — driven by column F of BankPosition tab
  const colB=d.cols[5];
  const companies=colB?[...new Set(allRows.map(r=>r[colB]).filter(Boolean))].sort():[];
  ...
  if (compSel) {
    ...
    compSel.onchange=function(){
      Sound.click();
      renderView(this.value?allRows.filter(r=>r[colB]===this.value):allRows);
    };
  }
  ...
  function renderView(rows) {
    ...
    renderKPIs('bank-kpis',[
      {icon:'📈',label:'Total Favorable Balance',   value:'Rs. '+fmtLKRFull(deposits), color:'green'},
      {icon:'📉',label:'Total Unfavorable Balance', value:'Rs. '+fmtLKRFull(liabs),    color:'red'},
      {icon:'⚖️',label:'Net Position',              value:'Rs. '+fmtLKRFull(net),      color:net>=0?'green':'red'},
      {icon:'🏦',label:'Bank Accounts',             value:rows.length,                 color:'blue'}
    ]);
    setTableCount('bank-count',rows.length);

    const sorted=[...rows].sort((a,b)=>(b['Amount']||0)-(a['Amount']||0));
    const bLabels=sorted.map(r=>abbrevBank(r['Bank Name']||''));
    const bData=sorted.map(r=>r['Amount']||0);
    Charts.bar('bank-bar',bLabels,[{label:'Balance (LKR)',data:bData,colors:bData.map(v=>v>=0?C.primary+'cc':C.red+'cc')}],{horizontal:true,currency:true});
    ...
    Charts.donut('bank-gauge',[net>=0?'Net Positive':'Net Negative','Offset'],
      [Math.abs(netPct),100-Math.abs(netPct)],{
        cutout:'72%',circumference:Math.PI,rotation:-Math.PI/2*180/Math.PI*-1,
        centerText:netPct.toFixed(0)+'%',centerSub:'Net ratio',
        centerColor:net>=0?C.green:C.red
    });

    // Portfolio by Bank Group — x-axis shows abbreviated bank names
    const bankGroups={};
    rows.forEach(r=>{
      const k=groupKey(r['Bank Name']||'Other');
...
async function initInvoicesPage() {
  ...
  // Column J (index 9) holds the invoice status: "Pending" or "Received"
  ...
  renderKPIs('inv-kpis',[
    {icon:'💼',label:'Total Invoice Value',   value:'Rs. '+fmtLKRFull(totalAmt),         color:'blue'},
  ...
  // Chart 1 — Age Analysis: PENDING invoices only
  ...
  // Chart 2 — Pending balance by Client (horizontal bar, sorted descending)
  ...
  // Chart 3 — Received vs Pending stacked by Client
  const clientMap={};
  rows.forEach(r=>{
    const ref=String(r['Client Ref']||'Unknown').split('/')[0];
...
async function initTenderPage() {
  ...
  // Populate site dropdown dynamically from actual sheet data
  ...
  // activeRows tracks what's currently displayed — used by table sort/search
  ...
  function renderView(rows) {
    ...
    // Chart 1 — Production Progress (adaptive by status filter)
    const selStatus=statusEl?.value;
    if (selStatus==='Completed') {
      // Completed view: show tender qty per completed tender
      ...
      // All / On-Going view: show % complete for on-going tenders
      ...
      if (og.length) {
        ...
        // No on-going in current filter — fall back to qty view
        const slice=rows.slice(0,12);
        Charts.bar('tender-progress',slice.map(r=>r['Tender No']||'—'),[{
          label:'Total Qty (poles)',
          data:slice.map(r=>r['Total Tender Qty']||r['Tender Qty']||0),
    ...
    // Chart 2 — Distribution by Site (count of tenders)
    const bySite={};
    ...
    // Chart 3 — Site Production Qty (dynamic — only sites present in filtered data)
  ...
  // Table sort/search wired ONCE — operates on activeRows so it respects current filter
  ...
  // Slicers — re-render ALL visuals with filtered rows
...
async function initSleepersPage() {
  ...
  function renderView(rows) {
    ...
    // Chart 1 — Production Progress
    const selStatus=statusEl?.value;
    if (selStatus==='Completed') {
      const slice=rows.slice(0,12);
      Charts.bar('slp-progress',slice.map(r=>r['Tender No']||'—'),[{
        label:'Total Qty (pcs)',
        data:slice.map(r=>r['Total Tender Qty']||0),
        colors:slice.map(()=>C.green+'cc')
      }],{unit:' pcs'});
    } else {
      const og=rows.filter(r=>r._tabStatus==='On-Going').slice(0,12);
      if (og.length) {
        const pcts=og.map(r=>{ const t=r['Total Tender Qty']||1,c=r['Total Qty Casted']||0; return Math.min(100,c/t*100); });
        Charts.bar('slp-progress',og.map(r=>r['Tender No']||'—'),[{
          label:'% Complete',
          data:pcts,
          colors:pcts.map(p=>p>=90?C.green+'cc':p>=50?C.primary+'cc':C.amber+'cc')
        }]);
    ...
    // Chart 2 — Distribution by Site
    const bySite={};
    rows.forEach(r=>{ const s=r['Site Name']||'Unknown'; bySite[s]=(bySite[s]||0)+1; });
    Charts.donut('slp-donut',Object.keys(bySite),Object.values(bySite));

    // Chart 3 — Casted vs Balance stacked by site
    const dynSites=[...new Set(rows.map(r=>r['Site Name']).filter(Boolean))];
    if (dynSites.length) {
...
async function initFillingPage() {
  ...
  rows._renderer=r=>{
    ...
    const cash=r['Cash Sales']||0, card=r['Card Sales']||0, credit=r['Credit Sales']||0;
    const total=cash+card+credit;
    const mix=total?`💵${Math.round(cash/total*100)}% 💳${Math.round(card/total*100)}%`:'—';
    return `<tr>
      <td>${fmtDate(r['Date'])}</td>
...
/* Creates one rising bubble inside a tank */
function spawnBubble(tankId) {
  const tank   = document.getElementById(`ftank-${tankId}`);
  if (!tank) return;
  const pct    = State.tankLevels[tankId] || 0;
  if (pct < 6)  return;                        // nearly empty — no bubbles

  const tankH  = tank.offsetHeight || 180;
  const fillH  = (pct / 100) * tankH;          // liquid height in px

  const el     = document.createElement('div');
  el.className = 'bubble';

  const size   = Math.random() * 5 + 1.5;      // 1.5 – 6.5 px diameter
  const leftPct= Math.random() * 55 + 12;      // 12 – 67 % from left
  /* start anywhere in the lower 40 % of the liquid column */
  const startBt= Math.random() * fillH * 0.4;
  /* rise between 55 % and 95 % of the fill height */
  const riseH  = -(fillH * (0.55 + Math.random() * 0.4));
  /* slight horizontal drift */
  const drift  = (Math.random() - 0.5) * 10;
  const dur    = Math.random() * 1.8 + 1.0;    // 1.0 – 2.8 s
...
/* Periodic bubble spawner for one tank (1–3 bubbles per cycle) */
function startBubbleLoop(tankId) {
  if (_bubbleIntervals[tankId]) return;          // already running
  _bubbleIntervals[tankId] = setInterval(() => {
    const count = Math.ceil(Math.random() * 2);  // 1 or 2 per tick
...
/* Apply shaking CSS class + optional level wobble to one tank */
function shakeTank(tankId, wobble = true) {
  ...
  if (wobble && fillEl) {
    ...
    const swing = 3 + Math.random() * 5;          // 3–8 % wobble amplitude
  ...
  /* burst of extra bubbles during shake */
  for (let i = 0; i < 4; i++) {
...
/* Shake all tanks (used on device-shake event) */
function shakeAllTanks() {
  Sound.liquid();
  TANK_IDS.forEach(id => shakeTank(id, true));
}

/* Wire up touch / click on each tank */
function initFuelTankEffects() {
  TANK_IDS.forEach(tankId => {
    startBubbleLoop(tankId);                       // start bubble spawner
...
function navigate(page) {
  ...
  if (target){ target.classList.remove('hidden'); requestAnimationFrame(()=>target.classList.add('page-active')); }
...
async function runSplash() {
  ...
  const steps=[[10,'Connecting…'],[30,'Fetching data…'],[55,'Building charts…'],[80,'Rendering UI…'],[100,'Ready']];
  try {
    ...
    setTimeout(()=>{ const s=document.getElementById('splash'); if(s)s.style.display='none'; },750);
  }
}
...
function initSwipe() {
  ...
  document.addEventListener('touchend',e=>{
    // Skip if swipe started inside scrollable/interactive areas
    ...
    // Require strong horizontal swipe (90px+) with horizontal dominant over vertical (2:1 ratio)
...
async function init() {
  ...
  document.querySelectorAll('.nav-item,.bnav-item').forEach(el=>el.addEventListener('click',e=>{ e.preventDefault(); navigate(el.dataset.page); }));
  ...
  navigate(PAGES.includes(hash)?hash:'bank');
...
init().catch(err=>{
  ...
  if (sp) sp.style.background='#1a0505';
});
